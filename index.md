---
layout: home
title: Welcome
# tagline: Staff Software Engineer / Software Architect
---

This is not a resume. This is a **deep dive** into my professional journey.

Expect to spend **2-3 hours** exploring:
- 🏗 **5 architectural specifications** (MHA, JEMP, SSA)
- 💼 **7 companies** with detailed project breakdowns
- 📝 **9 technical publications** (237K+ views)
- 🎓 **11 university lectures** (MIPT Go Course)
- 📦 **6 open source projects** (1,756 GitHub contributions)

[Start Exploring →](/about/)

---

## Quick Navigation

| Section | What You'll Find |
|---------|------------------|
| [About](/about/) | Engineering philosophy, career timeline |
| [Experience](/experience/) | 7 companies, project details |
| [Architecture](/architecture/) | MHA, JEMP, SSA specifications |
| [Publications](/publications/) | 9 Habr articles |
| [Projects](/projects/) | Open source on GitHub |
| [Contact](/contact/) | Get in touch |

---

## 📊 At a Glance

| Metric | Value |
|--------|-------|
| **Professional Experience** | 5+ years (2019–2026) |
| **Community Activity** | 9 years (Habr since 2017) |
| **Companies** | 7 (1C, MIND, MIPT, ForestVPN, Carrierproxy, VPN Manager, JetBrains) |
| **Publications** | 9 Habr articles (237K+ views, 13K+ karma) |
| **GitHub** | 1,756 contributions, 26 repositories |
| **Architecture Specs** | 5 documented specifications |
| **Lectures** | 11 (MIPT Go Course) |

---

> **Note:** All links are verified. All claims are backed by public evidence.
> 
> **Last Updated:** February 2026
